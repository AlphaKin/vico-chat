...
import org.vico.im.utils.CommonUtil;
...
public class MessageProcessor implements ImProcessor {
    @Override
    public Future<Object> execute(ChannelHandlerContext ctx, SessionManager sessionManager, AggregatedMessage aggregatedMessage) {
        ...
        val session = sessionManager.getBySessionId(aggregatedMessage.getSessionId());
        if(session == null){
            return new AsyncResult<>(false);
        }
        val aimSession = sessionManager.getByUserId(session.getImUser().getUserId());
